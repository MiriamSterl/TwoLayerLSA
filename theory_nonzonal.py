...
angle_UV = float(sys.argv[3])

# Include planetary beta effect in the calculation?
include_beta = sys.argv[4]
...
np.save('../Data/calculations/theory/'+filename+'_k.npy', k_inj)
np.save('../Data/calculations/theory/'+filename+'_sigma.npy', growth_rate_inj)
np.save('../Data/calculations/theory/'+filename+'_cr.npy', phase_speed_inj)
np.save('../Data/calculations/theory/'+filename+'_theta_cr.npy', theta_c)

print("Run with 1/mu = %i days, angle_UV = %i degrees, beta = %E finished"%(mu_days,angle_UV,beta))